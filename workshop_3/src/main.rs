...
pub struct School<T: Display + Debug + Ord + PartialEq> {
    students: HashMap<String, T>,
}

impl<T: Display + Debug + Ord + PartialEq> School<T> {
    pub fn new() -> Self {
        Self {
    ...
    pub fn add(&mut self, grade: T, student: &str) {
        ...
        // .expect("Error to add students");
    }

    pub fn grades(&self) -> Vec<&T> {
        let mut grade: Vec<&T> = Vec::new();
        for (index, student) in self.students.iter().enumerate() {
            if grade.contains(&student.1) {
                continue;
            }
            grade.insert(index, student.1);
        }
        grade.sort();
        grade
    }

    pub fn grade(&self, _grade: T) -> Vec<String> {
        let mut students_name: Vec<String> = Vec::new();
        for student in self.students.iter() {
            if student.1 == &_grade {
                students_name.push(student.0.to_string());
            }
        }
        // String already implement ORD traits
        students_name.sort();
        // println!("{:?}", students_name);
        students_name
    }
}

fn main() {
    let vec_student = vec![("Alice", 10), ("Bob", 2), ("Eve", 4), ("Long", 2)];
    let mut school: School<i32> = School::new();
    // Testcase 1: Empty hashMap
    println!("Student hashmap length: {:?}", school.students.len());
    for &student in &vec_student {
        school.add(student.1, student.0);
    }
    // Testcase 2: After adding
    println!("Students hashmap after adding {:?}", school.students);
    // Finding all grade order by asc
    let grade = school.grades();
    let student_names = school.grade(2);
    // Testcase 3: Finding student_name by grades order by asc
    println!("Grade: {:?}", grade);
    println!("Student have grade 2: {:?}", student_names);
}
